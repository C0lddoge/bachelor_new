...
def name_gen(N,L,lag,types):
    return 'RP_N{:},{:}_L{:}_mr_vh_lag{:}_type{:}.dat'.format(N[0],N[1],L,lag,types)
    ...
    for lind,L in enumerate(Ls2):
        ...
        for tyind,ty in enumerate(types):
            ...
            #r1 = return_concentration(N[1])/return_overlap(50,Rgs["50"]["30.0"])
                    ...
                    popt,pcov = cm.gauss_fit(data[data[:,0]>cutoff[lagind],:],times[lagind])
                    print(f'L = {L}; D = {popt}, ty = {ty}')
                    rs = np.linspace(cutoff[lagind],10)
                    axs[x][y].plot(rs,gauss(rs,*popt),color = colors[lag],linewidth = 1.5)
                    axs[x][y].plot(data[data[:,0]>cutoff[lagind],0],data[data[:,0]>cutoff[lagind],1],'x', markersize = 1.5, label = 'CG; $\\tau_l = {:}$'.format(lag), color = colors[lag],alpha = 0.6)
