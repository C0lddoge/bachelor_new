...
for nind,L in enumerate(Ls):
  ...
  for c, comb in enumerate([[0,0], [1,0], [1,1]]):
      ...
      if os.path.exists(data_dir + return_name_md(N[0],N[1],L,comb)):
        ...
        ax[x][y].errorbar(data[:,0], data[:,1],yerr = data[:,2],marker = 'x',linewidth = 0.0,elinewidth = 1.0, label = 'CG; {:}{:}'.format(comb[0]+1,comb[1]+1),markersize= 2.0, color = colors2['{:}{:}'.format(*comb)])
...
fig.delaxes(ax[3][1])
fig.savefig(expo_dir + 'gr_new.pdf')
