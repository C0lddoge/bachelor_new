...
def return_name(N,L,types):
    return 'RP_N{:},{:}_L{:}_MSDs_{:}_err.dat'.format(N[0],N[1],L,types)
...
for nind,L in enumerate(Ls):
  ...
  for ind,ty in enumerate(types):
      ...
      if os.path.exists(data_dir + return_name(N,L,ty)):
        print('test')
        data = np.loadtxt(data_dir + return_name(N,L,ty))
        data[:,1:] = data[:,1:]/Rgs[ind]**2
        err = data[:,3]
        std = data[:,2]
        ax[x][y].errorbar(data[:,0],data[:,1], yerr = err, marker ='o',linewidth = default_style["linewidth"], markersize = default_style["markersize"], label = labels[ty], color = colors[ty])
...
fig.delaxes(ax[3][1])
fig.savefig(expo_dir + 'msd_log.pdf')
