...
def simulate(N, density,T,xyz_switch,e_switch,p_switch):
    ...
    w_steps = 1000
    p_steps = 1000
    accept = cm.simulate_ana(N,eps,sig,beta,density,mc_moves,trunc,max_displacement,energy_steps,w_steps,p_steps,xyz_switch,e_switch,p_switch,xyz_file,E_tot_file,pressure_file)
    with open(sim_path + '/log.dat', 'w') as out_f:
        out_f.write('\ntrunc = {:}\nmc_moves = {:}\nenergy_steps = {:}\n xyz_steps = {:}\npressure_steps = {:}\nN = {:}\ndensity = {:}\nT = {:}\n accept_ratio = {:}'.format(trunc,mc_moves,energy_steps,w_steps,p_steps,N, density,T,accept))  
    out_f.close()
