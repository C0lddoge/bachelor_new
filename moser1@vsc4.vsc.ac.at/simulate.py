...
def simulate(N, density,T,xyz_switch,e_switch,p_switch,config_path):
    ...
    with open(sim_path + '/log.dat', 'w') as out_f:
        out_f.write('\ntrunc = {:}\nmc_moves = {:}\nenergy_steps = {:}\n xyz_steps = {:}\npressure_steps = {:}\nN = {:}\ndensity = {:}\nT = {:}\npotential_file = {:}'.format(trunc,mc_moves,energy_steps,w_steps,p_steps,N, density,T,config_path))  
    out_f.close()
    cm.simulate(N,eps,sig,beta,density,mc_moves,trunc,max_displacement,energy_steps,w_steps,p_steps,xyz_switch,e_switch,p_switch,potential_dict,xyz_file,E_tot_file,pressure_file)
