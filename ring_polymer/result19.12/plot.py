...
for nind,N in enumerate(Ns):
  ...
  for c, comb in enumerate([[0,0], [1,0], [1,1]]):
      ...
      if os.path.exists(data_dir + return_name_me(N[0],N[1],dens[nind],comb)):
        ...
        ax[nind].errorbar(data[:,0], data[:,1],yerr = data[:,2],label = '{:}{:}'.format(*comb), **default_style, color = colors['{:}{:}'.format(*comb)])

  ax[nind].set_title('$\\rho_0/\\rho_0^* = {:.4f}; \\rho_1/\\rho_1^* = {:.4f}; N_0 = {:}; N_1 = {:};$'.format(r0,r1,int(N[0]),int(N[1])))
