...
for nind,N in enumerate(Ns):
    for ty in types:
        if os.path.exists(data_dir + name_gen(N,ty,rhos[nind])):
            ...
            print(data)
            if ty == 'all':
                axs.bar(data[:,0],data[:,1],capsize = 1.5,width = widths, label = 'total cluster distribution')
                print(data[:,2])
                axs.errorbar(data[:,0],data[:,1], yerr = data[:,2],linewidth = 0.0, elinewidth = 1.0, capsize = 0.0, color = 'black')
            #else:
    ...
    axs.set_title('$N_f = {:}; N_s = {:};$'.format(int(N[0]),int(N[1])))
    axs.set_xlabel('rings per cluster',fontsize = 20)
    axs.set_ylabel('weight fraction',fontsize = 20)
    axs.set_ylim(10**(-5),1.1*10**0)
    #axs.legend(fontsize = 15)
    fig.savefig(export_dir + f'clustering_slides{nind}.pdf'.format(ty))
    axs.clear()
