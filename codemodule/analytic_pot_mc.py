    ...
    for i in range(N_total):
        if p != i:
            ...
            if d_squared<trunc**2:
                d = np.sqrt(d_squared)
                key = (types[p],types[i])
                energy_ += potential(d,eps[key],sig[key])
            else:
                energy_ += 0
    return energy_
        ...
        for j in range(i):
            if i != j:
                if d_arr[i,j]<trunc**2:
                    ...
                    key = (types[i],types[j])
                    energy_ += potential(d_arr[i,j],eps[key],sig[key])
                else:
                    energy_ += 0
...
def simulate_ana(N,eps,sig,beta,rho,mc_moves,trunc,max_displacement,e_step,w_step,p_step,xyz_switch,e_switch,p_switch,filename = '',energy_file = '',pressure_file = ''):
    N_total = np.sum(N)
    L = (N_total/rho)**(1/3)
